...
def extract_newcommands(tex):
  r"""
  string에서 newcommand 또는 renewcommand 추출
  추출 대상: \something, \(, \), \[, \]
  \newcommand, \renewcommand는 무조건 한 줄에 있다고 가정.
  \newcommand, \renewcommand 뒤에 주석은 없다고 가정.

  :param tex: string
  :return: dict (commands[name] = (n_args, default, body))
  """
  commands = {}
...
def extract_document(tex):
  r"""
  string에서 document만 추출한 뒤 주석 삭제.
  \%를 제외한 모든 %는 주석으로 간주.
...
def extract_title(tex):
  """
  string(tex 전체)중 document 부분에서 title 추출

  :param tex: string(tex 전체)
  :return: title
  """
...
def get_categories_from_path(relative_path):
  """
  relative_path를 categories 형식에 맞게 변환.
...
def extract_tags(tex):
  """
  string 형태의 tex로부터 tags를 추출.
  tags는 본운에 \tags{ ... } 형태로 정의되어 있다고 가정.
...
def extract_author(tex):
  """
  string 형태의 tex로부터 author를 추출.
  \author{...} 의 마지막 항목을 가져오며, 없거나 비어 있으면 None을 반환.
